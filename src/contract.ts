...
// The transport-free contract of the ctscout MCP tools (ctscout-mcp#72): every
// name, description, schema, annotation and renderer, shared by the stdio host
// (src/index.ts) and the hosted MCP in ctscout-worker. Nothing here reads the
// environment or fetches: a host supplies a CtscoutApi and a registrar.
 ...
 */
/** The calls a host makes on the contract's behalf. The stdio host binds them
 *  to ctscout.dev with an API key from the environment; the hosted MCP binds
 *  them to its own routes with the caller's key. The contract never fetches. */
export interface CtscoutApi {
  scan(body: ScanRequestBody): Promise<ScanResponse>;
  scanBatch(queries: ScanRequestBody[]): Promise<ScanBatchResponse>;
  submitJob(spec: DeepDiveSpec): Promise<JobSubmitResponse>;
  getJob(jobId: string): Promise<JobResponse>;
  leiRecord(lei: string): Promise<LeiRecord>;
  leiByName(name: string): Promise<LeiNameMatches>;
  vendorSummary(slug: string): Promise<VendorSummary>;
  vendorCustomers(slug: string): Promise<VendorCustomers>;
}

/** What a host offers for the contract to register on: the SDK v2
 *  `McpServer.registerTool` shape. An `McpServer` satisfies it directly; a
 *  host on another SDK adapts to it. */
export interface ToolRegistrar {
  registerTool: McpServer["registerTool"];
}

/** Register every ctscout tool on `server`, answering through `api`. This is
...
Legal-vs-brand and coverage caveats are identical to ctscout_search_company — brand names may need legal-entity variants ("X Companies", "X Group", "The X"), and coverage is best for established US/EU entities with OV/EV certs.`,
    ...
    async (params: SearchCompanyBatchInput) => {
      try {
        ...
        const raw = await api.scanBatch(queries);
...
Auth & limits: same as ctscout_search_company.`,
    ...
    async (params: LookupDomainInput) => {
      try {
        const raw = await api.scan({ seed_domain: params.domains });
        const data: ScanResponse = { ...raw, ...resolveSnapshot(raw) };
...
Examples:
  ...
  - Don't use when: you want an answer now — ctscout_search_company / ctscout_lookup_domain are synchronous. Don't resubmit while a job is queued or running; poll it.`,
    ...
    async (params: SubmitDeepDiveInput) => {
      ...
      try {
        const receipt = await api.submitJob(spec);
    ...
    async (params: LookupLeiInput) => {
      try {
        ...
        if (params.lei !== undefined) {
          const raw = await api.leiRecord(params.lei);
        ...
        const raw = await api.leiByName(params.name as string);
    ...
    async (params: VendorCustomersInput) => {
      try {
        if (params.enumerate) {
          ...
          const raw = await api.vendorCustomers(params.slug);
        ...
        const raw = await api.vendorSummary(params.slug);
